...
export function CheckoutPage() {
  ...
  if (!token) return <Navigate to={`/login?next=/subscribe/${planId}`} replace />
  ...
  return (
    <>
      <SeoHead
        title={`Subscribe to ${plan.name} - ${productName || APP_NAME}`}
        description={`Subscribe to ${plan.name} plan. ${plan.description}`}
      />
      <section className="mx-auto max-w-2xl px-4 py-16 sm:py-20">
        ...
        <div className="mt-8 rounded-2xl border border-surface-border bg-white p-6 dark:bg-zinc-900">
