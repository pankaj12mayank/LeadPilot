...
def _check_chrome_attach(port: int) -> bool:
    if not _port_open("127.0.0.1", port):
        ...
        _warn(
            "Or set ATTACH_EXISTING_CHROME=0 in .env (or scraper.env) to let Selenium start its own "
            "Chrome (no port 9222; set CHROME_USER_DATA_DIR for a profile, or log in in the new window)."
        )
        return False
