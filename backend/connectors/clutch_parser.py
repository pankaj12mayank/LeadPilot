"""Clutch.co provider / company pages from a DOM snapshot."""

from __future__ import annotations

import re
from typing import Any


def parse_clutch_snapshot(snap: dict[str, Any]) -> dict[str, Any]:
    url = str(snap.get("url") or "")
    h1 = str(snap.get("h1") or "").strip()
    doc_title = str(snap.get("title") or "").strip()
    text_sample = str(snap.get("textSample") or "")

    company = h1 or re.split(r"\s*[-—|]\s*", doc_title, maxsplit=1)[0].strip()

    website = ""
    wm = re.search(r"https?://[^\s)]+", text_sample)
    if wm:
        website = wm.group(0).strip().rstrip(".,);")

    email = ""
    em = re.search(r"[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}", text_sample, flags=re.I)
    if em:
        email = em.group(0)

    focus = ""
    m = re.search(r"(?im)service(?:s)? focus[:\s]+([^\n]+)", text_sample)
    if m:
        focus = m.group(1).strip()[:200]

    return {
        "name": "",
        "title": "",
        "company": company,
        "industry": focus,
        "location": "",
        "website": website,
        "email": email,
        "profile_url": url,
    }
